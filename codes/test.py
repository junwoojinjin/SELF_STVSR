...
import models.modules.Real_STVSR_Base as Real_STVR
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('--save_dir', '-o', nargs='*', help='Example) save', default='result', dest='save_dir')
parser.add_argument('--model_pth', nargs='*',type=str,  help='Example) save', default='result', dest='model_pth')
...
def main():
    ...
    #### model 
    #### TODO: change your model path her\
    model_path = args.model_pth[0]
    model = Real_STVR.Real_STVR(64, N_ot, 8, 5, 30)

    #### dataset
    data_mode = 'Vid4' #'Vid4' #'SPMC'#'Middlebury'#

    if data_mode == 'Vid4':
        test_dataset_folder = '/test_sequences_LR/LR/x4/*'
        #test_dataset_folder = '/mnt/ssd0/junwoojin/datas/vimeo_septuplet/test_sequences_LR/LR/x4/00096/*'

    if data_mode == 'Custom':
        test_dataset_folder ='/test_sequences_LR/LR/x4/00096/*'
    ...
    save_f_name = args.save_dir
    save_folder = '../results/{}'.format(save_f_name[0])
    ...
    sub_folder_LL = sorted(glob.glob(test_dataset_folder))
    for sub_folder_LG in sub_folder_LL:
        sub_folder_l = sorted(glob.glob(sub_folder_LG+ '/*'))
        avg_psnr_l = []
        avg_psnr_y_l = []
        sub_folder_name_l = []
        # total_time = []
